...
interface MyTree<E>{
    byte[] huffmanZip(byte[] bytes);
    byte[] zip(byte[] bytes,Map<Byte,String> huffmanCodes);
    Map<Byte,String> getCodes(Node root);
    void getCodes(Node node,String code,StringBuilder stringBuilder);
    void preOrder(Node root);
    List<Node> getNodes(byte[] bytes);
    String byteToBitString(boolean flag,byte t);
    byte[] decode(Map<Byte,String> huffmanCodes,byte[] huffmanBytes);
...
//        System.out.println("huffmanCodeBytes=" + Arrays.toString(huffmanCodeBytes));//17个
        ...
        public static void zipFile(String srcFile, String dstFile) {
            ...
            try {
                ...
                //别人给讲的，以对象流的方式写入哈弗曼编码，为了恢复文件时使用
